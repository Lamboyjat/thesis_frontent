...
export const Information = () => {
    ...
    function handleDownload() {
        ...
        element.download = `SMCP_Sentence_${new Date().toString()}.txt`
    ...
    return (
        <main className='flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 sm:gap-4 justify-center pb-20 w-full max-w-prose w-full mx-auto'>
            <h1 className='font-semibold text-4xl sm:text-5xl md:text-6xl whitespace-nowrap'><span className='text-blue-400 bold'>Your</span> Transcription</h1>
            ...
            <div className='grid grid-cols-2 sm:mx-auto bg-white rounded overflow-hidden items-center p-1 blueShadow border-[2px] border-solid border-blue-300'>
                ...
                <button onClick={() => setTab('translation')} className={'px-4 rounded duration-200 py-1  ' + (tab === 'translation' ? ' bg-blue-300 text-white' : ' text-blue-400 hover:text-blue-600')}>SMCP Sentence</button>
            </div>
            <div className='my-8 flex flex-col-reverse max-w-prose w-full mx-auto gap-4'>
                ...
                {tab === 'transcription' ? (
                    ...
                    <Smcp {...props} toLanguage={toLanguage} translating={translating} textElement={textElement} setTranslating={setTranslating} setTranslation={setTranslation} setToLanguage={setToLanguage} generateTranslation={generateTranslation} />
                )}
            </div>
